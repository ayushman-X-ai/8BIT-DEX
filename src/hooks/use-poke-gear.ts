...
export const usePokeGear = (canvasRef: React.RefObject<HTMLDivElement>) => {
  ...
  // Use a ref to hold the Pixi Application instance. This is the correct
  // way to manage non-React objects that should persist across renders
  // without triggering them.
  const appRef = useRef<Application | null>(null);

  useEffect(() => {
    // If there's no canvas element or the app is already created, do nothing.
    // This check prevents re-initialization if the component re-renders.
    if (!canvasRef.current || appRef.current) {
      return;
    }

    // Create the app instance and immediately store it in the ref.
    const app = new Application();
    appRef.current = app;

    // These are local to the effect's scope.
    ...
    const setup = async () => {
      try {
        ...
        // This is the only DOM manipulation, safe to do after init.
        ...
        timeline = gsap.timeline({
          onComplete: () => {
              // Check the ref to ensure the app wasn't destroyed mid-animation
              if (appRef.current) {
    ...
    // The cleanup function is the key. It's returned from useEffect and will
    // be called when the component unmounts.
    return () => {
      // Kill animations and stop sounds first
      timeline?.kill();
      sound?.stop();

      // Check if the app instance exists in the ref before destroying it.
      if (appRef.current) {
        appRef.current.destroy(true, true);
        appRef.current = null; // Important: nullify the ref after destruction.
      }
      setIsInitialized(false);
    };
  }, [canvasRef]); // The effect depends only on the canvas ref.
